...
/// 1. Try xterm CSI 14t query via /dev/tty (works with WezTerm panes)
/// 2. Fall back to TIOCGWINSZ ws_xpixel/ws_ypixel
/// 3. Fall back to cell count × estimated cell size
pub fn terminal_pixel_size() -> Option<(usize, usize)> {
    #[cfg(unix)]
    {
        // Method 1: xterm CSI 14t query — most reliable for split panes
        if let Some(size) = query_pixel_size_xterm() {
            return Some(size);
        }

        // Method 2/3: TIOCGWINSZ ioctl
        use std::os::unix::io::AsRawFd;
        let tty_fd = unsafe { libc::open(b"/dev/tty\0".as_ptr() as *const _, libc::O_RDWR) };
        let fds: &[i32] = if tty_fd >= 0 {
            &[tty_fd, std::io::stdout().as_raw_fd()]
        } else {
        ...
        for &fd in fds {
            ...
            if unsafe { libc::ioctl(fd, libc::TIOCGWINSZ, &mut ws) } == 0 {
                if ws.ws_xpixel > 0 && ws.ws_ypixel > 0 {
                    result = Some((ws.ws_xpixel as usize, ws.ws_ypixel as usize));
                    break;
                }
                if ws.ws_col > 0 && ws.ws_row > 0 {
                    let w = ws.ws_col as usize * 9;
                    let h = ws.ws_row as usize * 18;
                    result = Some((w, h));
                    break;
                }
            }
...
/// Query pixel size via xterm CSI 14t escape sequence.
/// Response format: ESC [ 4 ; <height> ; <width> t
#[cfg(unix)]
fn query_pixel_size_xterm() -> Option<(usize, usize)> {
    use std::io::{Read, Write};
    use std::os::unix::io::FromRawFd;

    let fd = unsafe { libc::open(b"/dev/tty\0".as_ptr() as *const _, libc::O_RDWR | libc::O_NOCTTY) };
    if fd < 0 {
        return None;
    }
    ...
    // Set raw mode: disable canonical + echo, short read timeout
    let mut raw = orig;
    raw.c_lflag &= !(libc::ICANON | libc::ECHO);
    raw.c_cc[libc::VMIN] = 0;
    raw.c_cc[libc::VTIME] = 2; // 200ms timeout
    unsafe { libc::tcsetattr(fd, libc::TCSANOW, &raw) };

    // Drain pending input
    unsafe { libc::tcflush(fd, libc::TCIFLUSH) };

    // SAFETY: we own this fd and will close it manually
    let mut tty = unsafe { std::fs::File::from_raw_fd(fd) };

    // Send CSI 14 t — request text area pixel size
    let sent = tty.write_all(b"\x1b[14t").is_ok() && tty.flush().is_ok();
    ...
    if sent {
        ...
        let mut len = 0;
        while len < buf.len() {
            match tty.read(&mut buf[len..len + 1]) {
                Ok(1) => {
                    if buf[len] == b't' {
                        len += 1;
                        break;
        ...
        if let Ok(s) = std::str::from_utf8(&buf[..len]) {
            if let Some(inner) = s.strip_prefix("\x1b[4;").and_then(|s| s.strip_suffix('t')) {
                let parts: Vec<&str> = inner.split(';').collect();
                if parts.len() == 2 {
                    if let (Ok(h), Ok(w)) = (parts[0].parse::<usize>(), parts[1].parse::<usize>()) {
                        if w > 0 && h > 0 {
                            result = Some((w, h));
                        }
                    }
    ...
    // Restore terminal — must use raw fd since File will close it on drop
    unsafe { libc::tcsetattr(fd, libc::TCSANOW, &orig) };
    // File drops here and closes fd
